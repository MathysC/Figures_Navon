...
def density():
    ...
    #Ajouter les Sous cases puis les espaces entre chaque sous matrice
    # Sous cases
    # Initialisation
    temp = {}
    ...
    for oldx in range(0, m1):
        for oldy in range(0, m2):
            ...
            for newx in range(0,dens):
                x = newx + dens * oldx + oldx*space
                for newy in range(0,dens): # Ajoute les colonnes de sous cases
                    y = newy + dens*oldy + oldy*space
                    temp[x,y] = NF[oldx, oldy]
                for esp in range(1,space+1): # Ajoute les espaces sous chaque colonne faite
                    temp[x,y+esp] = False
            for esp in range(1,space+1): # Il faut faire les colonnes d'espaces

                for tempY in range((dens+space)*oldy,(dens+space)*(oldy+1)): # Pour chaque ligne en
                    temp[x+esp,tempY] = False


    # Supprimer les dernières lignes des espaces ajoutés
    newX, newY = max(temp.keys())
    for i in range(newX - space + 1, newX + 1):
        for j in range(0, newY + 1):
            del temp[i, j]

    # Supprimer les dernières colonnes ajoutés
    newX, newY = max(temp.keys())  # MaJ des max X et Y

...
def impression():
    #rappelle de matrix pour avoir la figure créée la plus à jour
    ...
    for k2 in range(0, max(tabmatrix.keys())[0] + 1):
        for k in range(0, max(tabmatrix.keys())[1] + 1):
            draw.text((DEMI + TAILLEFONT*k,DEMI+TAILLEFONT*k2),LOCALCHAR if tabmatrix[k, k2] else " " ,(0,0,0),font=font)
    #Save de l'image
...
X_Scale.slider.pack(side = BOTTOM)

...
from FiguresNavon import FigureNavon
...
def sauvegarde():
    if not maFigureNavon.getFichierCharge():
        print("hey !")
        res_LettreGlobale = s_lGlobale.get()
        res_LettreLocale = s_lLocale.get()
        maFigureNavon.setElementGlobal(res_LettreGlobale)
...
def charge():
    filepathListeChargee = askopenfilename(initialdir="/", title="Open as", defaultextension="*.*",
...
# Bouton générer
bt_Generer = tkinter.Button(lf_sauvegardeForme, relief=RAISED, bg="#32CD32", fg="WHITE", text="Generate",
                            command=sauvegarde)
bt_Generer['font'] = f
bt_Generer.pack()
